...
export class FilterService {
  ...
  public async getAll(): Promise<FilterDefinition[]> {
    return lastValueFrom(this.client.getAll(new FilterServiceGetAllRequest())).then((f) =>
      this.mapFiltersFromResponse(f)
    );
  }

  public async getSingle(id: string): Promise<FilterDefinition> {
    return lastValueFrom(this.client.getSingle(this.mapToFilterGetSingleRequest(id))).then((f) =>
      this.mapFilterFromResponse(f)
    );
  }
  ...
  private mapToFilterVersionCreateRequest(
    filterId: string,
    name: string,
    deadline: Date
  ): FilterServiceCreateFilterVersionRequest {
  ...
  private mapToFilterVersionRenameRequest(
  ...
  public static mapFilterCriteria(filterCriteriaModel?: FilterCriteriaModel[]): FilterCriteria[] {
    ...
    filterCriteriaModel.forEach((c) => {
      const property: FilterCriteria = {
        id: c.id!,
        filterDataType: this.snakeToCamel(
          FilterDataType[c.filterDataType!].substring('FILTER_DATA_TYPE_'.length)
        ),
        filterOperator: this.mapFilterOperator(c.filterOperator),
        referenceId: this.referenceIdToString(c.referenceId!),
        filterValue:
          c.filterDataType === FilterDataType.FILTER_DATA_TYPE_MULTISELECT
            ? c.filterValue!.split(',')
            : c.filterValue!,
      };
  ...
  public static mapToFilterCriteria(criteria: FilterCriteria[]): FilterCriteriaModel[] {
    ...
    criteria.forEach((c) => {
      ...
      a.filterOperator =
        FilterOperator[
          ('FILTER_OPERATOR' +
            ConverterHelpers.splitCamelCaseToSnakeCase(c.filterOperator)) as keyof typeof FilterOperator
        ];
      a.filterDataType =
        FilterDataType[('FILTER_DATA_TYPE_' + c.filterDataType.toUpperCase()) as keyof typeof FilterDataType];
      a.referenceId =
        FilterReference[
          ('FILTER_REFERENCE' +
            ConverterHelpers.splitCamelCaseToSnakeCase(c.referenceId)) as keyof typeof FilterReference
        ];
      result.push(a);
    });
    return result;
  ...
  private static referenceIdToString(referenceId: FilterReference): string {
    return this.snakeToCamel(FilterReference[referenceId].substring('FILTER_REFERENCE_'.length));
  }
  ...
  private mapVersion(v: FilterVersionModel): FilterVersion {
    return {
      ...
      auditInfo: AuditInfoService.map(v.auditInfo!),
